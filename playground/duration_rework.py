...
class Durations():
  ...
  def add(self, name_, skillAttributes_ = [], duration_ = None, type_ = None):
    ...
    if name_ in data.getDurationData().keys() and data.getDurationData()[name_]['type'] == 'damagingAilment':
      tmpStats = stats.Stats(self._durations['buff'])
    else:
    ...
    if duration.getType() != 'cooldown' and data.getDurationData()[duration.getName()]['maxStack'] != 0 and self.countActiveByName(duration.getName())[duration.getName()] >= data.getDurationData()[duration.getName()]['maxStack']:
  ...
  def countActiveByType(self, *type_):
    active = {}
    for a in self.getActiveByType(*type_):
  ...
  def countActiveByName(self, *name_):
    ...
    for a in self.getActiveByName(*name_):
      active[a.getName()] = active.get(a.getName(), 0) + 1
    return active

  # count all active durations
  def countActive(self):
    return self.countActiveByType()
  ...
  def tick(self, timestep_ = 0.1, boss_ = False):
    ...
    shreds = self.countActiveByType('shred')
    for key in shreds:
